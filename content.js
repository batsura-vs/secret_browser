...
help_button.style.position = 'fixed'
help_button.style.opacity = '0.5'
help_button.style.right = '0'
help_button.style.bottom = '0'
help_button.innerText = 'Update'
help_button.style.zIndex = '100000000000000000000'
help_button.style.border = 'none'
help_button.style.background = 'none'
help_button.style.outline = 'none'
help_button.style.cursor = 'pointer'
...
function encrypt(message = '') {
    message = CryptoJS.AES.encrypt(message, key.value);
    return message.toString();
}

function decrypt(message = '') {
    var code;
    code = CryptoJS.AES.decrypt(message, key.value);
    try {
...
function search_child(element, text) {
    var str = element.innerHTML.replaceAll(/<.*>*?<\/.*>/g, '');
    for (var i = 0, atts = element.attributes, n = atts.length, arr = []; i < n; i++) {
        var att = atts[i]
        if (att.nodeValue.includes(text)) {
...
help_button.onclick = function () {
    ...
    var last;
    do {
        var text = document.body.innerHTML.match(new RegExp("(!##(\\/?[^>]+)##!)", 'g'));
        if (text) {
            if (text[0] !== last) {
                last = text[0];
            } else {
            ...
            let message = text[0].replace('!##', '').replace('##!', '');
            var elements = document.body.children
            for (var i = 0; i < elements.length; i++) {
                var res = search_child(elements[i], text[0]);
                if (res === true) {
                    if (decrypt(message)) {
                        elements[i].innerHTML = elements[i].innerHTML.replaceAll(text[0], decrypt(message))
                    } else {
                        elements[i].innerHTML = '!!НЕВОЗМОЖНО РАСШИФРОВАТЬ!!'
                        elements[i].style.background = '#f87474'
                    }
                    break
                } else if (res !== false) {
                    elements = res
    ...
    } while (text && repeats < 20)


    var inputs = document.querySelectorAll('div[contenteditable]')
    for (var i = 0; i < inputs.length; i++) {
        inputs[i].addEventListener('input', function () {
            var en = this.innerHTML.match(new RegExp("`!`(.*)`!`"));
            if (en) {
                let message = en[1];
                this.innerHTML = this.innerHTML.replace(en[0], `!##${encrypt(message)}##!`)
            }
        })
    }
    ...
    for (var i = 0; i < inputs.length; i++) {
        console.log(1);
        inputs[i].addEventListener('input', function () {
            var en = this.value.match(new RegExp("`!`(.*)`!`"));
            if (en) {
                let message = en[1];
                this.value = this.value.replace(en[0], `!##${encrypt(message)}##!`)
            }
        })
    }
